...
def assertz_mandatory(
    ...
    if isinstance(_element, str) and not _element:
        ...
        return assertz_call(
            _element in _obj and _element is not None and _obj[_element] is not None,
        ...
        return assertz_call(
            _obj, f"Mandatory: {_obj}", _error_code, _status_code, reply=reply
        )
...
def assertz_string(
    _element,
    key=None,
    _error_code=INVALID_TYPE_ERROR_CODE,
    _status_code=DEFAULT_STATUS_CODE,
    reply=False,
):
    if _element is None:
        return None
...
def assertz_integer(
    ...
    if isinstance(_element, int) and key is None:
        ...
        return assertz_call(
            key in _element and isinstance(_element[key], int),
            f"Invalid value type: {_element[key]}",
            _error_code,
            _status_code,
            reply=reply,
        )
...
def assertz_float(
    ...
    if isinstance(_element, float) and key is None:
        ...
        return assertz_call(
            key in _element and isinstance(_element[key], float),
            f"Invalid value type: {_element[key]}",
            _error_code,
            _status_code,
            reply=reply,
...
def assertz_timestamp(
    ...
    if (
        _element is None
        or isinstance(_element, datetime.date)
        or isinstance(_element, datetime.date)
    ):
        return None
    ...
    if key is None:
        ...
        return assertz_call(
            key in _element and Now.to_datetime(_element[key]),
            f"Invalid value type: {_element[key]}",
            _error_code,
            _status_code,
            reply=reply,
...
def assertz_boolean(
    ...
    if isinstance(_element, bool) and key is None:
        ...
        return assertz_call(
            key in _element and isinstance(_element[key], bool),
            f"Invalid value type: {_element[key]}",
            _error_code,
            _status_code,
            reply=reply,
...
def assertz_array(
    ...
    try:
        ...
        return assertz_call(
            False, f"Invalid value type: {key}", _error_code, _status_code, reply=reply
        )
...
def assertz_complex(
    ...
    return assertz_call(
        None in [is_json, is_dict, is_array],
        msg,
        _error_code,
        _status_code,
        reply=reply,
...
def assertz_regex(
    ...
    try:
        return assertz_call(
            re.match(regex_expr, value).group() == value,
            _error_msg,
            _error_code,
            _status_code,
        )
...
def assertz_uri(
    ...
    return assertz_regex(
        URI_REGEX, "Invalid uri", _element, key, _error_code, _status_code, reply=reply
    )
...
def assertz_url(
    ...
    return assertz_regex(
        URL_REGEX, "Invalid url", _element, key, _error_code, _status_code, reply=reply
    )
    ...
    try:
        if isinstance(value, dict):
            _expression = (
                ...
                and (
                    isinstance(value["latitude"], int)
                    or isinstance(value["latitude"], float)
                )
                and (
                    isinstance(value["longitude"], int)
                    or isinstance(value["longitude"], float)
                )
            )
        ...
        return assertz_call(
            _expression, _error_msg, _error_code, _status_code, reply=reply
        )
...
def assertz_intersects(
    ...
    return assertz_call(
        _expression,
        f"No intersection between {x} and {y}",
        _error_code,
        _status_code,
        reply=reply,
    )
...
def assertz_unauthorized(
    _expression, _error_msg, _error_code=INVALID_TYPE_ERROR_CODE, reply=False
):
    return assertz_call(
        _expression, _error_msg, _error_code, status_codes.unauthorized, reply=reply
    )


def assertz_valid_values(
    _expression,
    _error_msg,
    _error_code=INVALID_TYPE_ERROR_CODE,
    _status_code=DEFAULT_STATUS_CODE,
