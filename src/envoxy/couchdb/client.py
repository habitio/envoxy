...
class Client:
    ...
    def connect(self, instance):
        ...
        Log.trace(
            ">>> Connected to COUCHDB: {}, {}".format(instance["server"], _conf["bind"])
        )
        ...
        for _key, _value in (params or {}).items():
            try:
                ...
                if _operator in self.valid_operators:
                    _selector[_key] = {"${}".format(_operator): _value}
                    continue
            except IndexError:
                pass
    ...
    def _execute_request(self, session, method, url, data, retries=3, backoff=1):
        for _attempt in range(retries):
            try:
                ...
                if Log.is_gte_log_level(Log.DEBUG):
                    Log.debug(
                        "CouchDB::execute_request - Request took {:.2f} seconds".format(
                ...
                if _attempt < retries - 1:
                    Log.warning(
                        "CouchDB::execute_request - Retrying in {} seconds... Retries: {}/{}".format(
                            backoff, _attempt + 1, retries
                        )
                    )
                    time.sleep(backoff)
    ...
    def base_request(self, db, method, data=None, find=False, uri=None):
        ...
        if not _session:
            Log.warning(
                'CouchDB::base_request - No session found for server: "{}" to "{}"'.format(
                    _server_key, _url
                )
            )
        ...
        try:
            if _response:
                if (
                    ...
                    and "docs" in _response.json()
                ):
                    return _response.json()["docs"]

                Log.warning(
                    "CouchDB::find - Different response than expected - status code: {}, content: {}".format(
                        _response.status_code, _response.text
                    )
                )
            ...
            Log.warning(
                "CouchDB::find - Empty response / no session / no connection - DB: {} - Data: {}".format(
                    db, _data
                )
            )
    ...
    def get(self, id: str, db: str):
        ...
        try:
            if _response:
                ...
                Log.warning(
                    "CouchDB::get - Different response than expected - status code: {}, content: {}".format(
                        _response.status_code, _response.text
                    )
                )
            ...
            Log.warning(
                "CouchDB::get - Empty response / no session / no connection- DB: {} - URI/ID: {}".format(
                    db, id
                )
            )
    ...
    def post(self, db: str, payload: dict):
        ...
        try:
            if _response:
                if (
                    _response.status_code == requests.codes.created
                    and "docs" in _response.json()
                ):
                    return _response.json()["docs"]

                Log.warning(
                    "CouchDB::post - Different response than expected - status code: {}, content: {}".format(
                        _response.status_code, _response.text
                    )
                )
            ...
            Log.warning(
                "CouchDB::post - Empty response / no session / no connection - DB: {} - Payload: {}".format(
                    db, payload
                )
            )
