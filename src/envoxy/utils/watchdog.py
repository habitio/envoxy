...
class Watchdog:
    ...
    def start(self):
        if self.interval is not None and self.interval > 0:
            ...
            log.verbose(
                "[{}] not enabled, keep_alive missing or 0".format(
                    log.style.apply("Watchdog", log.style.GREEN_FG)
                )
            )
    ...
    def send_notification(self):
        try:
            ...
            while not event.wait(self.interval - 1):
                ...
                if main_thread_alive:
                    if (
                        ...
                        log.verbose(
                            "[{}] Watchdog sent successfully".format(
            ...
            log.error(
                "[{}] Error {}".format(log.style.apply("Watchdog", log.style.RED_FG), e)
            )
        except ImportError:
            log.warning(
                "[{}] systemd not imported {}".format(
                    log.style.apply("Watchdog", log.style.RED_FG),
                    traceback.format_exc(limit=5),
                )
            )
