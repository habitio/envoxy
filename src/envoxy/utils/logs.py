...
class LogStyle:
    ...
    def generate(styles):
        return "\033[{}m".format(
            styles if not isinstance(styles, list) else ";".join(styles)
        )
    ...
    def apply(text, styles=DEFAULT_FG):
        if text is not None:
            return "".join(
                [LogStyle.generate(styles), text, LogStyle.generate(LogStyle.RESET)]
            )
...
class Log:
        ...
        if Log.is_gte_log_level(Log.EMERGENCY):
            ...
            uwsgi.log(
                Log.format_log(
                    "{} | {} | {}".format(
                        LogStyle.apply(
                            " emergency ", [LogStyle.RED_BG, LogStyle.WHITE_FG]
                        ),
    ...
    def verbose(text, prefix=True, max_lines=None):
        if Log.is_gte_log_level(Log.VERBOSE):
            ...
            if prefix:
                uwsgi.log(
                    Log.format_log(
                        Log.truncate_text(text, max_lines), Log.VERBOSE, caller
                    )
                )
            else:
                uwsgi.log(
    ...
    def format_log(text, log_level, caller):
        ...
        _exec_name = (
            Config.get("boot")[0]["name"]
            if Config and len(Config.get("boot"))
