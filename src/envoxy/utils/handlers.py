...
class Handler:
    ...
    def response(response: RequestsResponse) -> FlaskResponse:
        try:
            return FlaskResponse(
                response.text, response.status_code, headers=response.headers.items()
            )
    ...
    def make_response(object_, status) -> FlaskResponse:
        if type(object_) in [dict, list]:
            return FlaskResponse(
                envoxy_json_dumps(object_), status, {"Content-Type": "application/json"}
            )
        elif type(object_) in [str]:
            return FlaskResponse(object_, status, {"Content-Type": "text/html"})
        else:
            return FlaskResponse(
                envoxy_json_dumps({"text": "Error in parsing the response object."}),
                500,
    ...
    def freeze(object_):
        if isinstance(object_, dict):
            return frozenset(
                (key, Handler.freeze(value)) for key, value in object_.items()
            )
